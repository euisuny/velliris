...
Section fundamental_exp.
  ...
  Lemma expr_logrel_EXP_Ident :
    forall (id : LLVMAst.ident) (dt : option dtyp) C i_t i_s A_t A_s,
      ...
      exp_conv (denote_exp dt (EXP_Ident id))
      [{ (v_t, v_s), uval_rel v_t v_s ∗ code_inv C i_t i_s A_t A_s }].
  Proof.
   ...
   Lemma expr_logrel_ind_case :
    ...
    □ (∀ x : dtyp * exp dtyp,
      ...
      → ∀ (a : option dtyp) (a0 : gmap (vir.loc * vir.loc) Qp),
            code_inv a0 i_t i_s A_t A_s -∗
            exp_conv (denote_exp a x.2) ⪯
            exp_conv (denote_exp a x.2)
            [{ (v_t, v_s),uval_rel v_t v_s ∗ code_inv a0 i_t i_s A_t A_s }]) -∗
      (code_inv C i_t i_s A_t A_s) -∗
      r <- exp_conv
      ...
      iAssert (□
        (∀ x : dtyp * exp dtyp,
          ...
          ∀ (a : option dtyp) (a0 : gmap (vir.loc * vir.loc) Qp),
            ...
            [{ (v_t, v_s),uval_rel v_t v_s ∗ code_inv a0 i_t i_s A_t A_s  }]))%I as "Helts".
  ...
  Theorem expr_logrel_refl dt e C A_t A_s :
    (⊢ expr_logrel C (denote_exp dt e) (denote_exp dt e) A_t A_s )%I.
  Proof.
