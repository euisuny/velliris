...
Section instruction_laws.
  ...
  Lemma sim_call_instr C dt x_t x_s f args_t args_s i_t i_s L_t L_s P Q:
      ...
      x_t ∉ L_t ->
      x_s ∉ L_s ->
      (∀ (l_t l_s : loc) (q : Qp), C !! (l_t, l_s) = Some q → (q < 1)%Qp) ->
      checkout C -∗
      frame_WF i_t i_s -∗
      ldomain_tgt i_t L_t -∗ ldomain_src i_s L_s -∗
      stack_tgt i_t -∗ stack_src i_s -∗
      P -∗
      instr_conv (translate exp_to_instr (denote_exp None f))
      ⪯ instr_conv (translate exp_to_instr (denote_exp None f))
     ...
     (P -∗
      ldomain_tgt i_t L_t -∗ ldomain_src i_s L_s -∗
      stack_tgt i_t -∗ stack_src i_s -∗
      instr_conv (Util.map_monad
        (λ '(t, op), translate exp_to_instr (denote_exp (Some t) op)) args_t)
        ⪯
      instr_conv (Util.map_monad
          (λ '(t, op), translate exp_to_instr (denote_exp (Some t) op)) args_s)
      [{ (args_t, args_s),
          ([∗ list] x;y ∈ args_t; args_s, uval_rel x y) ∗ Q ∗
          ldomain_tgt i_t L_t ∗ ldomain_src i_s L_s ∗
          stack_tgt i_t ∗ stack_src i_s }]) -∗

      <{ %(IId x_t) = (INSTR_Call (dt, f) args_t [FNATTR_Readonly]) }> ⪯
      <{ %(IId x_s) = (INSTR_Call (dt, f) args_s [FNATTR_Readonly]) }>
    [{ (v_t,v_s),
        ∃ v_t' v_s', Q ∗
        stack_tgt i_t ∗ stack_src i_s ∗
        ldomain_tgt i_t ({[x_t]} ∪ L_t) ∗
        ldomain_src i_s ({[x_s]} ∪ L_s) ∗
        [ x_t := v_t' ]t i_t ∗ [ x_s := v_s' ]s i_s ∗
        uval_rel v_t' v_s' ∗
        checkout C }].
  Proof.
    iIntros (Ht Hs Hpre) "Hc #WF Hd_t Hd_s Hs_t Hs_s HP Haddr Hargs".
    repeat setoid_rewrite instr_conv_bind.

    iApply sim_expr_bind.
    iApply (sim_expr_bupd_mono with "[Hc Haddr]");
      last (iApply ("Hargs" with "HP Hd_t Hd_s Hs_t Hs_s")); try done.
    cont.
    iApply sim_expr_bind.
    iDestruct "H" as "(HP & HQ & Hd_t & Hd_s & Hs_t & Hs_s)".

    iApply (sim_expr_bupd_mono with "[HP HQ Hd_t Hd_s Hs_t Hs_s Hc]");
      last iApply "Haddr".
    iDestruct "HP" as "#H'".
    cont.
    ...
    rewrite /instr_conv.
    iApply (sim_expr_bupd_mono with "[HQ Hc Hd_t Hd_s Hs_t Hs_s]");
    ...
    iPoseProof (call_readonly_internal with "WF Hs_t Hs_s Hc Hdv H'") as "H".
    { simp call_invariant; cbn.
      exists READONLY; do 2 (split; eauto). }

    iApply (sim_expr_bupd_mono with "[HQ Hd_t Hd_s]"); [ | iApply "H"].

    cbn. cont.
    iDestruct "H" as "(Hs_t & Hs_s & #Hv' & HC)".
    iApply sim_expr_base.
    rewrite !bind_ret_l.
    iApply sim_expr_tau; iApply sim_expr_base.
    rewrite !bind_ret_l.
    ...
    iApply sim_expr_vis.
    rewrite !subevent_subevent.
    iApply (sim_expr_bupd_mono with "[HQ HC]");
      [ |
        iApply (sim_local_write_alloc _ _ _ _ _ _ _ _ Ht Hs
            with "Hd_t Hd_s Hs_t Hs_s")].
    cont.
    iApply sim_expr_tau.
    iApply sim_expr_base.

    iDestruct "H" as "(H_t & H_s & Hd_t & Hd_s & Hs_t & Hs_s )".

    iExists _, _; do 2 (iSplitL ""; first done); iFrame.
    iExists v_t0, v_s0; iFrame. done.
  Qed.
...
Section more_properties.
  ...
  Lemma call_argmemonly_internal C dt args_t args_s u_t u_s i_t i_s l_t l_s q v_t v_s:
    attribute_interp (ExternalCall dt u_t args_t [FNATTR_Argmemonly])
                     (ExternalCall dt u_s args_s [FNATTR_Argmemonly]) C ->
    frame_WF i_t i_s -∗
    stack_tgt i_t -∗
    stack_src i_s -∗
    checkout C -∗
    ...
    trigger (ExternalCall dt u_t args_t [FNATTR_Internal; FNATTR_Argmemonly]) ⪯
    trigger (ExternalCall dt u_s args_s [FNATTR_Internal; FNATTR_Argmemonly])
    [{ (x,y),
        l_t ↦{q}t v_t ∗
        l_s ↦{q}s v_s ∗
        stack_tgt i_t ∗
  ...
  Proof.
    intros Hcall.
    iIntros "#WF Hst Hss Hc Hl_t Hl_s #Haddr Hargs".
    ...
    - iApply sim_coindF_coind_bind.
      ...
      iIntros (??). destruct v_t0, v_s0; simp vir_call_ans.
      iIntros "H"; iFrame.
  ...
  Lemma call_argmemonly_internal1 C dt args_t args_s u_t u_s i_t i_s l_t l_s q v_t v_s:
    attribute_interp (ExternalCall dt u_t args_t [FNATTR_Argmemonly])
                     (ExternalCall dt u_s args_s [FNATTR_Argmemonly]) C ->
    frame_WF i_t i_s -∗
    stack_tgt i_t -∗
  ...
  Lemma sim_call_argmemonly_instr1
    C dt x_t x_s f p_t p_s i_t i_s L_t L_s P Q l_t l_s v_t v_s q:
      (* SSA should ensure that this would hold *)
      x_t ∉ L_t ->
      x_s ∉ L_s ->
      (∀ (l_t l_s : loc) (q : Qp), C !! (l_t, l_s) = Some q → (q < 1))%Qp ->
     ...
     (P -∗
      ...
      stack_tgt i_t -∗ stack_src i_s -∗
      instr_conv (Util.map_monad
        (λ '(t, op), translate exp_to_instr (denote_exp (Some t) op)) p_t)
        ⪯
      instr_conv (Util.map_monad
      ...
      [{ (args_t, args_s),
          ...
          ldomain_tgt i_t L_t ∗ ldomain_src i_s L_s ∗
          stack_tgt i_t ∗ stack_src i_s }]) -∗
      <{ %(IId x_t) = (INSTR_Call (dt, f) p_t [FNATTR_Argmemonly]) }> ⪯
      <{ %(IId x_s) = (INSTR_Call (dt, f) p_s [FNATTR_Argmemonly]) }>
    [{ (a, b),
        ∃ v_t' v_s', Q ∗
        stack_tgt i_t ∗ stack_src i_s ∗
        l_t ↦{q}t [ v_t ] ∗
        ...
        checkout C }].
  Proof.
    iIntros (Ht Hs Hpre) "Hl_t Hl_s #WF Hd_t Hd_s Hs_t Hs_s Hc HP Haddr Hargs".
    repeat setoid_rewrite instr_conv_bind.

    iApply sim_expr_bind.
    iApply (sim_expr_bupd_mono with "[Hc Haddr Hl_t Hl_s]");
      last (iApply ("Hargs" with "HP Hd_t Hd_s Hs_t Hs_s")); try done.
    cont.
    iApply sim_expr_bind.
    iDestruct "H" as "(HP & %HQ & HQ & Hd_t & Hd_s & Hs_t & Hs_s)".

    iApply (sim_expr_bupd_mono with "[HP HQ Hd_t Hd_s Hs_t Hs_s Hc Hl_t Hl_s]");
      last iApply "Haddr".
    cont.
    ...
    rewrite /instr_conv.
    iApply (sim_expr_bupd_mono with "[HQ Hc Hd_t Hd_s Hs_t Hs_s HP Hl_t Hl_s]");
      last (iApply (instr_conv_concretize_or_pick_strong with "Hv")).
    ...
    iPoseProof
      (call_argmemonly_internal1 with
                "WF Hs_t Hs_s Hc Hl_t Hl_s Hdv HP") as "H".
    { simp call_invariant; cbn.
      exists ARGMONLY; do 2 (split; eauto). }

    iApply (sim_expr_bupd_mono with "[HQ Hd_t Hd_s]"); [ | iApply "H"].

    cbn. cont.
    iDestruct "H" as "(Hl_t & Hl_s & Hs_t & Hs_s & #Hv' & HC)".
    iApply sim_expr_base.
    rewrite !bind_ret_l.
    iApply sim_expr_tau; iApply sim_expr_base.
    rewrite !bind_ret_l.
    ...
    iApply sim_expr_vis.
    rewrite !subevent_subevent.
    iApply (sim_expr_bupd_mono with "[HQ HC Hl_t Hl_s]");
      [ |
        iApply (sim_local_write_alloc _ _ _ _ _ _ _ _ Ht Hs
            with "Hd_t Hd_s Hs_t Hs_s")].
    cont.
    ...
    iExists _, _; do 2 (iSplitL ""; first done); iFrame.
    iExists x, y; iFrame. done.
  Qed.

  Lemma sim_call_argmemonly_instr
    C dt x_t x_s f p_t p_s i_t i_s L_t L_s P Q l_t l_s v_t v_s q:
      (* SSA should ensure that this would hold *)
      x_t ∉ L_t ->
      x_s ∉ L_s ->
